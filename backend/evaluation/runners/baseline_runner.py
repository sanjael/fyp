"""
Baseline RAG pipeline runner.

Uses the EXACT same retriever (ChromaDB + Ollama embeddings) and LLM as
RAGGuard-TR, but skips RRFE, TRRI, and the risk decision gate entirely.
...
from app.services.vector_store import get_chroma_client as _get_chroma_client
...
def _collection_name(sample_id: str) -> str:
    ...
    return f"eval_bl_{safe}"[:63]


class BaselineRAGRunner:
    """
    Vanilla RAG: Retrieve → Concatenate → Generate.
    No RRFE, no TRRI, no risk decision.
    """
    ...
    def run(self, sample: BenchmarkSample) -> PipelineResult:
        ...
        try:
            ...
            # 3. Generate answer directly — no RRFE, no TRRI
            ...
            result.error = str(e)
            logger.error(f"Baseline pipeline failed for {sample.sample_id}: {e}")
        finally:
